...
pub fn Calendar() -> impl IntoView {
    ...
    let current = Calendar::from(current.year(), current.month(), current.date.day());

    view! {
        <div class="calendar">
            <div class="calendar-header">
                <span class="month-name">{format!("{} {}", current.month(), current.year())}</span>
            ...
            <div class="calendar-grid">
                {weekdays_names()} {previous_month_visible_days(&current)}
                {current_month_days(&current)} {next_month_visible_days(&current)}
            </div>
        </div>
    }
}
